...
function drawGraphBackground(canvas, config, bounds) {
  ...
  var overflowPaint = {
    color: GRAPH_COLORS.gridOverflow,
    stroke: true,
    strokeWidth: 1,
  };
...
export function drawCurve(canvas, currentEasing, config, lastEasing) {
  ...
  if (lastEasing) {
    ...
    var lastCurvePaint = {
      color: GRAPH_COLORS.lastCurve || "#4c4c4c",
      stroke: true,
      strokeWidth: 2,
    };
  ...
  if (endpointPath && endpointPath.toObject) {
    canvas.addPath(endpointPath.toObject(), {
      color: "#ffffff",
      stroke: false,
    });
  }
...
export function drawSpeedCurve(canvas, currentEasing, speedEasing, config) {
  ...
  var sampleCount = 50;
  var velocitySamples = sampleVelocityCurve(
    x1Clamped,
    y1,
    x2Clamped,
    y2,
